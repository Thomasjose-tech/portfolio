...
//     const touchDiffX = touchStartX.current - touchEndX.current;
//     const touchDiffY = touchStartY.current - touchEndY.current;
//     const minSwipeDistance = 100; // Increased threshold for more intentional swipes
    
//     // Only trigger navigation if it's a clear horizontal swipe (more horizontal than vertical movement)
//     if (Math.abs(touchDiffX) > minSwipeDistance && Math.abs(touchDiffX) > Math.abs(touchDiffY) * 1.5) {
//       if (touchDiffX > 0) {
//         // Swipe left - next page
//         handleNextPage();
//       } else {
...
const Certifications = () => {
  ...
  // Touch event handlers with smooth swipe effects
  const handleTouchStart = (e) => {
  ...
  const handleTouchMove = (e) => {
    ...
    // Only apply offset if horizontal movement is dominant
    if (Math.abs(touchDiffX) > Math.abs(touchDiffY)) {
      // Limit the offset to prevent over-swiping
      const maxOffset = 100;
      const limitedOffset = Math.max(-maxOffset, Math.min(maxOffset, -touchDiffX * 0.5));
  ...
  const handleTouchEnd = () => {
    ...
    const touchDiffX = touchStartX.current - touchEndX.current;
    const touchDiffY = touchStartY.current - touchEndY.current;
    const minSwipeDistance = 50; // Reduced from 100px for easier swiping
    
    // Only trigger navigation if it's a clear horizontal swipe
    if (Math.abs(touchDiffX) > minSwipeDistance && Math.abs(touchDiffX) > Math.abs(touchDiffY) * 1.2) {
      if (touchDiffX > 0) {
        // Swipe left - next page
        handleNextPage();
      } else {
    ...
    <section id="certifications" className="py-20 px-4 bg-background">
      <div className="container mx-auto">
          ...
          <div className="text-center mb-6 animate-fade-in">
            ...
            <div className="mt-2 flex justify-center">
              <div className="flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full">
                <div className="w-1 h-1 bg-primary rounded-full animate-pulse" style={{ animationDelay: '0s' }}></div>
        ...
        <div className="space-y-8 relative overflow-hidden">
          <div 
            ref={containerRef}
            className={`grid gap-6 ${isMobile ? 'grid-cols-1' : 'md:grid-cols-2 lg:grid-cols-3'} transition-all duration-300 ease-out`}
            style={{
              transform: isMobile && !isTransitioning ? `translateX(${swipeOffset}px)` : 'translateX(0)',
          ...
          >
            {currentCertifications.map((cert, index) => (
              ...
              >
                <Card 
                  className={`bg-gradient-card shadow-elegant border-0 hover:shadow-glow transition-all duration-500 hover:-translate-y-2 group animate-fade-in h-full flex flex-col ${
                    isDragging && isMobile ? 'cursor-grabbing scale-[0.98]' : ''
                  } ${isTransitioning ? 'blur-[1px]' : ''}`}
                  style={{ 
                    animationDelay: `${index * 0.1}s`,
                    transform: isDragging && isMobile ? 'scale(0.98)' : '',
                  }}
                >
                  {/* Certificate Image */}
                  <div className="relative overflow-hidden rounded-t-lg">
